...
export default class ActionPoint{
  ...
  moveButton(){
    this.time.on('tick', () => {
      ...
      this.vector = this.vector.setFromMatrixPosition( this.model.children[this.childrenValue].matrixWorld )
      this.vector.project(this.camera.children[0])

      this.vector.x = ( this.vector.x * this.sizes.viewport.width/2 ) + this.sizes.viewport.width/2;
